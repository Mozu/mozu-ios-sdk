                   ...
                   completionHandler:(MOZUUserAuthenticationCompletionBlock)completion
{
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setValue:@"application/json" forHTTPHeaderField:@"content-type"];
    [request setValue:@"1.17.15154.0" forHTTPHeaderField:@"x-vol-version"];
    [request setHTTPMethod:@"POST"];
    if (guestAuthTicket != nil){
        [request setValue:guestAuthTicket.accessToken forHTTPHeaderField:MOZU_X_VOL_USER_CLAIMS];
    }
    
    NSDictionary *userAuthDict = @{ @"username": userAuthInfo.emailAddress, @"password": userAuthInfo.password };
    NSData *body = [NSJSONSerialization dataWithJSONObject:userAuthDict options:0 error:nil];
